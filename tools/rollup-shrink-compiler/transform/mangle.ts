...
import { Range } from "../parsing/types";
...
export class Mangle {
  ...
  public mangle = (name: string, sourceId: string): string => {
    ...
    if (stored && stored !== mangled) {
      console.log("SetIdentifier for Mangled Name more than once", {
        name,
        sourceId,
      });
    } else {
  ...
  public execute = async (
    ...
    const mangleable: Array<Set<string>> = [
      new Set([...this.nameToMangled.keys()]),
    ];
